...
class myQueries:
    ...
    def tokenQuery(self,qText):

        st = nltk.stem.PorterStemmer() # Porter Algorithm Stemming
        #start cleanning text
        word_tokens = nltk.wordpunct_tokenize(qText.lower())  # parse to single tokens and perform lowercase
    ...
    def queryResults(self, queryText ):
        ...
        biqDic = Counter(bigrams(self.tokenQuery(queryText)) )
        triqDic = Counter(trigrams(self.tokenQuery(queryText)))
